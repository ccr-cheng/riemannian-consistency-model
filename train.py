...
@click.option('--manifold',      help='Which manifold the data is on', metavar='STR',               type=click.Choice(['Torus', 'Sphere', 'SO3', 'Euclidean']), default='Torus', show_default=True)
...
def main(**kwargs):
    ...
    if opts.precond == 'flow':
        ...
        c.loss_kwargs.class_name = 'training.loss.FlowLoss'
    ...
    # Description string.
    desc = f'{opts.dataset_name}-{opts.precond:s}-gpus{dist.get_world_size():d}-batch{c.batch_size:d}'
